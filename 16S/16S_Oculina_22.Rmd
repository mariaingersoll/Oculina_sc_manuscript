...
```{r}
setwd("~/Desktop/BU_Research/Davies-Gilmore/Oculina_sc_Manuscript/Oculina_sc_Manuscript/16S")
```
...
```{r}
path <- "/Users/mariaingersoll/Desktop/BU_Research/Davies-Gilmore/16S_Oculina_22" # CHANGE ME to the directory containing the fastq files after unzipping.
fns <- list.files(path)
...
```{r}
meta <- read.csv("16S_Oculina_metadata.csv")
meta
...
```{r}
fnFs_cut <- file.path(path, "filtN", paste0(sample_names, "_16S_R1.fastq")) # 16S path with the subdirectory 'cutadapt' then the sample names
fnFs_cut
fnRs_cut <- file.path(path, "filtN", paste0(sample_names, "_16S_R2.fastq"))
...
filt_out
 #                 reads.in reads.out
#A5_16S_R1.fastq    21357     20664
#A6_16S_R1.fastq    21478     20700
#B6_16S_R1.fastq    10073      9400
#C4_16S_R1.fastq     1009       963 blank
#D4_16S_R1.fastq      858       688 blank
#D5_16S_R1.fastq    13139     12756
#E4_16S_R1.fastq     2407      2259 blank
#E6_16S_R1.fastq     4624      4474 blank
#E7_16S_R1.fastq    10646     10406 blank
#F4_16S_R1.fastq    14345     13808
#F5_16S_R1.fastq    13400     13083
#G4_16S_R1.fastq    21389     20679
#G5_16S_R1.fastq    65019     63666
#H4_16S_R1.fastq    14825     14497
#H5_16S_R1.fastq    13305     12932

####this is where I pulled the "mean depth of coverage of XX +/- XX"
...
#170 sequence variants were inferred from 4338 input unique sequences.
#Key parameters: OMEGA_A = 1e-40, OMEGA_C = 1e-40, BAND_SIZE = 16

######totaling the "unique sequences" from each of these for the "a total of XX sequences..."
```
...
dadaRs[[1]]
#dada-class: object describing DADA2 denoising results
#259 sequence variants were inferred from 6907 input unique sequences.
#Key parameters: OMEGA_A = 1e-40, OMEGA_C = 1e-40, BAND_SIZE = 16
```
...
dim(seqtab) # 17 samples, 1238 sequence variants
...
save as a csv
```{r}
write.csv(track_reads,file="ReadFilterStats_AllData_final.csv",row.names=TRUE,quote=FALSE)
```
...
```{r}
silva_path <- "~/Desktop/BU_Research/Davies-Gilmore/16S_Oculina_22/silva_nr99_v138.1_train_set.fa.gz" 
...
```{r}
silva_sp_path <- "~/Desktop/BU_Research/Davies-Gilmore/16S_Oculina_22/silva_species_assignment_v138.1.fa.gz"
...
write.csv(taxa_sp, file="taxa_sp.csv",row.name=TRUE,quote=FALSE)
...
#phyloseq-class experiment-level object
#otu_table()   OTU Table:         [ 1150 taxa and 15 samples ]
#sample_data() Sample Data:       [ 15 samples by 4 sample variables ]
#tax_table()   Taxonomy Table:    [ 1150 taxa by 7 taxonomic ranks ]
...
#[1]  27  36  44  61  74  80  91  98 103 139 166 199 204 219 250 271 272 331 428 683
```
...
ps_cleaner 
# phyloseq-class experiment-level object
#otu_table()   OTU Table:         [ 1130 taxa and 10 samples ]
#sample_data() Sample Data:       [ 10 samples by 5 sample variables ]
#tax_table()   Taxonomy Table:    [ 1130 taxa by 7 taxonomic ranks ]
```
...
#[1] "zscores:"
#named numeric(0)
#[1] "OTUs passing frequency cutoff  1e-04 : 664"
#[1] "OTUs with counts in 0.02 of samples:"

#TRUE 
# 664 
seq_trim <- seq_trim_allinfo[,3:664] 
...
phylo_trim 
#phyloseq-class experiment-level object
#otu_table()   OTU Table:         [ 662 taxa and 10 samples ]
#sample_data() Sample Data:       [ 10 samples by 5 sample variables ]
#tax_table()   Taxonomy Table:    [ 662 taxa by 8 taxonomic ranks ]
...
phylo_trim_rare <- phyloseq(otu_table(seqtab_trim_rare, taxa_are_rows=FALSE), 
                    ...
                    tax_table(taxa2))
phylo_trim_rare # 662 taxa remain
...
write.csv(metageneassist_input, "metageneassist_input_043023.csv", quote=F,row.names = FALSE)
write.csv(meta_input, "meta_no_blank.csv", quote=F,row.names = FALSE)

write.table(seqtab_trim_rare_t, file="seqtab_trim_rare_042723.tsv", quote=F, sep="\t", row.names = FALSE)
```
...
trim_rare_rel_phyla <- plot_bar(phylo_trim_rare_rel, x = "sample_id", fill="Phylum")+
  ...
  scale_fill_manual(values = colorRampPalette(brewer.pal(11, "Paired"))(28)) +
  scale_colour_manual(values = colorRampPalette(brewer.pal(11, "Paired"))(28)) +
...
trim_rare_rel_phyla
# saved as rarefied_total_ASVs_abundance.pdf as landscape 8x5.5
...
#trim_rare_rel_phyla_sym.pdf as 8x5
...
proteo_plot_sym <- plot_bar(phylo_proteo_rel, x = "sym_state", fill = "Order")+
  ...
  ggtitle("Proteobacteria ASVs (relative abundance)")
proteo_plot_sym
#saved as proteobacteria_ASV_abundance_sym.pdf as landscape 10x6
...
bact_plot_sym <- plot_bar(phylo_bact_rel, x = "sym_state", fill = "Order")+
  ...
  ggtitle("Bacteroidota ASVs (relative abundance)")
bact_plot_sym
##saved as bacteroidota_ASV_abundance_sym.pdf as landscape 8x6
...
#Groups     1 0.002292 0.0022921  0.0864 0.7763
#Residuals  8 0.212172 0.0265215    
...
#Response: Distances
#          Df   Sum Sq  Mean Sq F value    Pr(>F)    
#Groups     4 0.045684 0.011421 3.7525e+29 < 2.2e-16 ***
...
#relative abundance = the percentage of the mircobiome that is made up by a specific ASV
...
phylo_trim_rare # 662 rarefied and trimmed across all 10 samples
...
# checking the model
check_normality(simp_lm) #OK: residuals appear as normally distributed (p = 0.643)
...
# genet    (Intercept) 0.002887 0.05373 
# Residual             0.001008 0.03174 
#Number of obs: 10, groups:  genet, 5

#Fixed effects:
#             Estimate Std. Error t value
#(Intercept)   0.07788    0.02791   2.790
#sym_statesym  0.02439    0.02008   1.215
...
## checking the model
check_normality(shan_lm) # OK: residuals appear as normally distributed (p = 0.562).
...
#Random effects:
# Groups   Name        Variance Std.Dev.
# genet    (Intercept) 0.02697  0.1642  
# Residual             0.01116  0.1056  
#Number of obs: 10, groups:  genet, 5

#Fixed effects:
#             Estimate Std. Error t value
#(Intercept)   1.25748    0.08733  14.400
#sym_statesym -0.04338    0.06682  -0.649
...
## checking the model
check_normality(rich_lm) # OK: residuals appear as normally distributed (p = 0.145).
...
#Random effects:
# Groups   Name        Variance  Std.Dev.
# genet    (Intercept) 3.057e-06 0.001748
# Residual             2.698e-06 0.001642
#Number of obs: 10, groups:  genet, 5

#Fixed effects:
#               Estimate Std. Error t value
#(Intercept)   0.0089601  0.0010728   8.352
#sym_statesym -0.0004892  0.0010388  -0.471
...
## checking the model
check_normality(even_lm) #OK: residuals appear as normally distributed (p = 0.648).
...
#Random effects:
# Groups   Name        Variance Std.Dev.
# genet    (Intercept) 0.008421 0.09176 
# Residual             0.003583 0.05986 
#Number of obs: 10, groups:  genet, 5

#Fixed effects:
#             Estimate Std. Error t value
#(Intercept)   0.74652    0.04900  15.236
#sym_statesym -0.03485    0.03786  -0.921
...
plot2 <- plot_bar(ps.top90, x = "sample_id", fill="Class")+
  ...
  theme_bw()+
  theme(panel.grid = element_blank()) +
  scale_fill_manual(values = colorRampPalette(rev(brewer.pal(11, "Paired")))(18)) +
  scale_colour_manual(values = colorRampPalette(rev(brewer.pal(11, "Paired")))(18)) +
...
plot4 <- plot_bar(ps.top20, x = "sample_id", fill="Genus")+
  ...
  theme_bw()+
  theme(panel.grid = element_blank()) +
  scale_fill_manual(values = colorRampPalette(rev(brewer.pal(11, "Paired")))(17)) +
  scale_colour_manual(values = colorRampPalette(rev(brewer.pal(11, "Paired")))(17)) +
  facet_wrap(~sym_state, ncol = 2, scales="free") +
...
```{r}
seqtab_trim_rare_t <- read.delim("seqtab_trim_rare_042723.tsv")
head(seqtab_trim_rare_t)
