...
summary(aov(PC_2 ~ sym_state, data = pca_gd1)) #0.89
summary(aov(PC_3 ~ sym_state, data = pca_gd1)) #<2e-16 ***
summary(aov(PC_4 ~ sym_state, data = pca_gd1)) #2.13e-11 ***
summary(aov(PC_5 ~ sym_state, data = pca_gd1)) #<2e-16 ***                             
summary(aov(PC_6 ~ sym_state, data = pca_gd1)) #5.42e-10 ***
summary(aov(PC_7 ~ sym_state, data = pca_gd1)) #0.991
summary(aov(PC_8 ~ sym_state, data = pca_gd1)) #1.41e-08 ***
summary(aov(PC_9 ~ sym_state, data = pca_gd1)) #0.323
summary(aov(PC_10 ~ sym_state, data = pca_gd1)) #3.74e-05 ***
...
myd88.2_feat_plot_g1 <- FeaturePlot(gderm1, features=c("MYD88.2"), split.by = "symstate", order = TRUE, reduction="umap",  min.cutoff = 1) + theme(legend.position = c(0.98,0.98)) #marker for cluster 3 but not very strong
...
catl.9_feat_plot_g2 <- FeaturePlot(gderm2, features=c("CATL.9"), split.by = "symstate", order = TRUE, reduction="umap",  min.cutoff = 1) + theme(legend.position = c(0.98,0.98))
catl.7_feat_plot_g2 <- FeaturePlot(gderm2, features=c("CATL.7"), split.by = "symstate", order = TRUE, reduction="umap",  min.cutoff = 3) + theme(legend.position = c(0.98,0.98))
...
mmp24_feat_plot_g2 <- FeaturePlot(gderm2, features=c("MMP24"), split.by = "symstate", order = TRUE, reduction="umap",  min.cutoff = 3) + theme(legend.position = c(0.98,0.98))
sodc_feat_plot_g2 <- FeaturePlot(gderm2, features=c("SODC.4"), split.by = "symstate", order = TRUE, reduction="umap",  min.cutoff = 2) + theme(legend.position = c(0.98,0.98))

h90_feat_plot_g2 <- FeaturePlot(gderm2, features=c("H90A1"), split.by = "symstate", order = TRUE, reduction="umap",  min.cutoff = 3) + theme(legend.position = c(0.98,0.98))
h90a_vln1_gderm2 <- VlnPlot(object =gderm2, features = c("H90A1"), pt.size= 0, layer="data", combine = F,assay = "RNA", group.by = "symstate", cols=c("gray40", "#CC6600"))
h90a_vln2_gderm2 <- VlnPlot(object =gderm2, features = c("H90A1"), pt.size= 0,combine = F,assay = "RNA", split.by = "symstate", cols=c("gray40", "#CC6600"))
...
make a list of nf-kb response genes you're interested in
```{r}
#from oculina transcriptomics: TLR2, "TENR","TENR.11","TENR.18"
# from oc_full_aposym_mark: CATL.9, H90A1, L2EFL.1, HSP71, MMP10, HSP16
...
symapo_imm <- c("TLR2", "TENR","TENR.11","TENR.18", "CATL.9", "H90A1", "L2EFL.1", "HSP71", "MMP10", "HSP16", "DLG1","DLG5", "TM175","TM175.1","TM175.2", "VAMP1", "VAMP4", "VAMP7", "DLG1", "DLG5", "SFXN1", "CTRB","CTRB.3","CTRB.4","CTRB.6","CTRB.7","CTRB.8", "RAB6", "CRYAB", "LGMN", "PDXK", "PSMG2", "PLOD1", "PDIA4", "CATL","CATL.1","CATL.2","CATL.3","CATL.4","CATL.6","CATL.7","CATL.8", "PYG")
...
nfkb_perc_bin <- DimPlot(oc_diff_2, reduction = "umap", group.by = "NFKB_perc_bin", cols=c("grey80", "blue"), split.by = "symstate", order=c(">2%","<2%"))

nfkb_perc_cont <- FeaturePlot(oc_diff_2, reduction = "umap", features="NFKB_percent", order=TRUE, min.cutoff = 1.5, split.by = "symstate") + theme(legend.position = c(0.98,0.98)) 

nfkb_perc_vln <- VlnPlot(oc_diff_2, features = "NFKB_percent", split.by = "symstate")
...
oc_diff_3 <- PercentageFeatureSet(oculina_named_new, features = oc_new_overlap_other$other_imm, col.name = "general_imm_percent")
...
```{r}
################# HAVEN'T DONE BELOW YET, this is still for gderm2 tho
#get apo vs sym differences for subcluster 1 of gderm1
...
plot_cells(oculina.cds,
           color_cells_by = "cluster",
           label_groups_by_cluster=FALSE,
           label_leaves=TRUE,
           label_branch_points=TRUE)

plot_cells(oculina.cds, show_trajectory_graph = FALSE, color_cells_by = "partition")
...
plot_cells(sym.cds, show_trajectory_graph = FALSE, color_cells_by = "partition")
...
Split to just gastrodermal cells
```{r}
...
gderm.cds <- learn_graph(gderm.cds, use_partition = TRUE)
...
gderm_sym.cds <- learn_graph(gderm_sym.cds, use_partition = TRUE)
...
gderm_apo.cds <- learn_graph(gderm_apo.cds, use_partition = TRUE)
