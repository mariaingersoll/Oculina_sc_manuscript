...
ultimate_merge <- subset(merge, features = rownames(counts_names))
```
...
markers_fullnames <- markers_fullnames %>%
  ...
  write_csv('./files/Oculina_clean_markers_091724_logFC.5.csv')
...
nrow(markers_strong)
#new 7089
write_csv(markers_strong, "./files/SuppDataset_oculina_markers_strong_091724.csv")
...
#oculina_new is the one that was PCA-ed and UMAP-ed without the symbiont reads
oculina_new <- merge
```
...
saveRDS(oculina_named_new, paste0(proj_name, '_oculinanamed_NEW_091724.rds'))
...
#umap_oculina_091724 as landscape 8.5x6
...
#umap_oculina_aposym_091724 as landscape 6.5x5.5
```
...
saveRDS(top10_lev_new, paste0(proj_name, '_top10_releveled_named_091724.rds'))
...
nrow(top20_lev_new)
#540
saveRDS(top20_lev_new, paste0(proj_name, '_top20_releveled_named_091724.rds'))
...
#saved as top10_releveled_named_091724 as landscape 9x6
...
#saved as top20_releveled_named_091724 as landscape 9x6
```
...
gderm2 <- FindClusters(gderm2, resolution = 0.5)
#3 cluster
...
gderm2@active.ident <- factor(x = gderm2@active.ident, levels = 1:4)
gderm2@active.ident

levels(gderm2) <- c("1", "2", "3", "4")

plot7 <- DimPlot(gderm2, reduction = "umap", label = T, cols=c("#CC6699", "#33CC99", "#3333FF", "#FF9900"), alpha=0.5)
#4 clusters
...
map(paste0("PC_", 1:10), function(pc){
        ...
        plot_grid(plotlist = .)
#saved as gderm2_pcplots_umap_091724
...
summary(aov(PC_1 ~ sym_state, data = pca_gd2)) #1.07e-05 ***
summary(aov(PC_2 ~ sym_state, data = pca_gd2)) #<2e-16 ***
summary(aov(PC_3 ~ sym_state, data = pca_gd2)) #0.247
summary(aov(PC_4 ~ sym_state, data = pca_gd2)) #2.81e-11 ***
summary(aov(PC_5 ~ sym_state, data = pca_gd2)) #4.05e-08 ***                         
summary(aov(PC_6 ~ sym_state, data = pca_gd2)) #0.0639 .
summary(aov(PC_7 ~ sym_state, data = pca_gd2)) #0.0765 .
summary(aov(PC_8 ~ sym_state, data = pca_gd2)) #0.00169 **
summary(aov(PC_9 ~ sym_state, data = pca_gd2)) #0.0239 *
summary(aov(PC_10 ~ sym_state, data = pca_gd2)) #0.0917 .
...
#I could do findmarkers to define the difference between the algal-hosting cells and the other gderm2 cells
