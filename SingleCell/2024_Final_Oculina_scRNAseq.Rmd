...
NOT THIS WAY: -doing it with the GO_swiss_prot from the annotations (apoculata_GeneAnnotation_combined.txt) file 
```{r}
#BP
input="go_input_full2.csv"
...
```{r}
go_bp_all <- read.csv("./MWU_BP_go_input_full2.csv", header=TRUE, sep=" ")
head(go_bp_all)
...
```{r}
bp_all = read.csv("./BP_go_input_full2.csv", sep="\t")
head(bp_all)
...
write.csv(go_input_gderm1.2, file="./go_input_gderm1.2.csv", quote=F, row.names=FALSE)
...
try with the swissprot GO terms NAHHH
```{r}
#BP
input="go_input_gderm1.2.csv"
goAnnotations="apoculata_swissprot_iso2go.txt"
...
#go_gderm1_bp_swiss as portrait 20x10 still not great; could highlight what genes are involved in immune processes from the pc feats
```


Reformat GO BP output tables
```{r}
go_bp_gderm1 <- read.csv("./MWU_BP_go_input_gderm1.2.csv", header=TRUE, sep=" ")
head(go_bp_gderm1)
...
```{r}
bp_gderm1 = read.csv("./BP_go_input_gderm1.2.csv", sep="\t")
head(bp_gderm1)
...
DoHeatmap(IC, features = top20_named_imm,label=T, group.colors = c("#CC6699", "#33CC99", "#3333FF")) + theme(axis.text.y= element_text(size=5)) + scale_fill_gradientn(colors=color)
...
12. Neuron 1 RES OF 0.3 ####HERE DEC 10!
```{r}
...
gderm_sym_plot_cells <- plot_cells(gderm_sym.cds,
           ...
           label_leaves=TRUE,
           label_branch_points=TRUE)
#saved as gderm_sym_trajectory as portrait 5x5
...
gderm_apo_plot_cells <- plot_cells(gderm_apo.cds,
           ...
           label_leaves=TRUE,
           label_branch_points=TRUE)
#saved as gderm_apo_trajectory as portrait 5x5
```
...
write.csv(ocsym_shared, file="ocsym_shared_GO.csv", quote=F, row.names=FALSE)
```
...
gd_shared <- gd_shared %>%
  ...
  mutate(Fishers = if_else(is.na(Fishers), 0, 1)) %>%
  select(Protein_ID, Fishers)
write.csv(gd_shared, file="gd1and2_shared_GO.csv", quote=F, row.names=FALSE)
```
...
write.csv(fj_ocsym_only, file="ocsym_ONLY_GO.csv", quote=F, row.names=FALSE)
...
write.csv(fj_xen_only, file="xen_ONLY_GO.csv", quote=F, row.names=FALSE)
...
ocsymXEN_shared <- ocsymXEN_shared %>%
  ...
  mutate(Fishers = if_else(is.na(Fishers), 0, 1)) %>%
  select(Protein_ID, Fishers)
write.csv(ocsymXEN_shared, file="ocsymXEN_shared_GO.csv", quote=F, row.names=FALSE)
```
...
#ocsym_ONLY_GO_fisherBP 12x10 portrait #these could be supp tables mentioned in the text; this might all go in the supps
```

```{r}
input="xen_ONLY_GO.csv"
...
gd1and2XEN_shared <- gd1and2XEN_shared %>%
  ...
  mutate(Fishers = if_else(is.na(Fishers), 0, 1)) %>%
  select(Protein_ID, Fishers)
write.csv(gd1and2XEN_shared, file="gd1and2XEN_shared_GO.csv", quote=F, row.names=FALSE)
```
...
fj_gd1and2_only <- fj_gd1and2_only %>%
  ...
  mutate(Fishers_oculina = if_else(is.na(Fishers_oculina), 0, 1)) %>%
  select(Protein_ID, Fishers_oculina)
write.csv(fj_gd1and2_only, file="fj_gd1and2_ONLY_GO.csv", quote=F, row.names=FALSE)
...
fj_gd_xen_only <- fj_gd_xen_only %>%
  ...
  mutate(Fishers_xenia = if_else(is.na(Fishers_xenia), 0, 1)) %>%
  select(Protein_ID, Fishers_xenia)
write.csv(fj_gd_xen_only, file="fj_gd_xen_ONLY_GO.csv", quote=F, row.names=FALSE)
