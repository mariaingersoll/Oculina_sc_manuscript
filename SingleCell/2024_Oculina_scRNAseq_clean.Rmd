...
annotations <- read.delim("./new_Ap_ref/apoculata_GeneAnnotation_combined.txt", sep="\t", header=TRUE)
head(annotations)
...
seq2iso <- annotations %>%
  ...
  select(Protein_ID, gene)
head(seq2iso)
write.table(seq2iso, file="astrangia_seq2iso_new.txt", quote=F, sep="\t", row.names=FALSE)
...
head(seq2iso2gene)
write.table(seq2iso2gene, file="astrangia_seq2iso2gene_new.txt", quote=F, sep="\t", row.names=FALSE)
...
markers_fullnames <- markers_fullnames %>%
  ...
  write_csv(paste0(proj_name, '_clean_markers_new_logFC.5.csv'))
